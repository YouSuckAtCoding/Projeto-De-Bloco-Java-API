INSERT INTO users (Name,  Email, Password)
VALUES ("Admin", "admin@teste.com", "123456");

INSERT INTO products (Name, Price, Description) VALUES
('Product 1', 19.99, 'Description for product 1'),
('Product 2', 29.99, 'Description for product 2'),
('Product 3', 39.99, 'Description for product 3'),
('Product 4', 49.99, 'Description for product 4'),
('Product 5', 59.99, 'Description for product 5'),
('Product 6', 69.99, 'Description for product 6'),
('Product 7', 79.99, 'Description for product 7'),
('Product 8', 89.99, 'Description for product 8'),
('Product 9', 99.99, 'Description for product 9'),
('Product 10', 109.99, 'Description for product 10');